...
fn buildPrototypesModule(root: *Root, log_mod: *std.Build.Module, cimgui_mod: *std.Build.Module) !*std.Build.Module {
    ...
    native_source.appendSlice(root.builder.graph.arena, dir.readFileAlloc(root.builder.graph.io, root.builder.pathResolve(&.{ "src", "native.zig" }), root.builder.graph.arena, .unlimited) catch @panic("OOM")) catch @panic("OOM");
    native_source.append(root.builder.graph.arena, 0) catch @panic("OOM");

    prototypes_source.appendSlice(root.builder.graph.arena,
        \\const std = @import("std");
        \\const c = @import("c");
    ...
    var set = std.BufSet.init(root.builder.graph.arena);
    ...
    while (token.tag != .eof) {
        if (precedent[precedent.len - 1] != null) {
            if (std.mem.eql(u8, native_source.items[precedent[1].?.loc.start..precedent[1].?.loc.end], "prototypes") and precedent[0].?.tag == .period and std.mem.startsWith(u8, native_source.items[token.loc.start..token.loc.end], "vk") and !set.contains(native_source.items[token.loc.start..token.loc.end])) {
                prototypes_source.appendSlice(root.builder.graph.arena, "var _") catch @panic("OOM");
                prototypes_source.appendSlice(root.builder.graph.arena, native_source.items[token.loc.start..token.loc.end]) catch @panic("OOM");
                prototypes_source.appendSlice(root.builder.graph.arena, ": c.PFN_") catch @panic("OOM");
                prototypes_source.appendSlice(root.builder.graph.arena, native_source.items[token.loc.start..token.loc.end]) catch @panic("OOM");
                prototypes_source.appendSlice(root.builder.graph.arena, " = null;\npub var ") catch @panic("OOM");
                prototypes_source.appendSlice(root.builder.graph.arena, native_source.items[token.loc.start..token.loc.end]) catch @panic("OOM");
                prototypes_source.appendSlice(root.builder.graph.arena, " : @typeInfo(c.PFN_") catch @panic("OOM");
                prototypes_source.appendSlice(root.builder.graph.arena, native_source.items[token.loc.start..token.loc.end]) catch @panic("OOM");
                prototypes_source.appendSlice(root.builder.graph.arena, ").optional.child = undefined;\n") catch @panic("OOM");
...
fn buildLogModule(root: *Root, ffi_imports_mod: *std.Build.Module, pages_build: bool) *std.Build.Module {
    ...
    return root.builder.createModule(.{
        ...
        .imports = &.{
            .{ .name = "build", .module = options.createModule() },
            .{ .name = "imports", .module = ffi_imports_mod },
        },
    });
}
...
fn buildFFIModule(root: *Root, shader_types_mod: *std.Build.Module, log_mod: *std.Build.Module, ffi_imports_mod: *std.Build.Module, options_mod: *std.Build.Module) *std.Build.Module {
    ...
    return root.builder.createModule(.{
        ...
        .single_threaded = true,
        .imports = &.{
            .{ .name = "c", .module = translate_constants.mod },
            .{ .name = "build", .module = options_mod },
            .{ .name = "imports", .module = ffi_imports_mod },
...
fn buildTraceLibrary(root: *Root, log_mod: *std.Build.Module) *std.Build.Step.Compile {
    ...
    const trace_mod = root.builder.createModule(.{
        ...
        .single_threaded = true,
        .imports = &.{
            .{ .name = "c", .module = translate_trace.mod },
            .{ .name = "log", .module = log_mod },
        },
    });
...
fn buildWASMCImGuiModule(root: *Root, op_lib: *std.Build.Step.Compile, trace_lib: *std.Build.Step.Compile) !*std.Build.Module {
    ...
    cimgui_lib.root_module.addCSourceFile(.{ .file = cimgui_builder.path(cimgui_builder.pathResolve(&.{ "dcimgui", "master", "backends", "imgui_impl_wgpu.cpp" })), .flags = &webgpu_flags });
    cimgui_lib.root_module.addCSourceFile(.{ .file = cimgui_builder.path(cimgui_builder.pathResolve(&.{ "dcimgui", "master", "backends", "dcimgui_impl_wgpu.cpp" })), .flags = &webgpu_flags });

    // Here we are patching an imgui source file directly. For an unknown reason, g_ContextMap (from imgui_impl_glfw.cpp) resets
    // after initialization and makes the backend crashes when used later. If the map is used into a static function, the reset doesn't happen.
    // This workaround should be deleted as soon as this usecase will be fixed into the Zig compiler for WASM target.
    var imgui_impl_glfw_cpp = try cimgui_builder.root.root_dir.handle.readFileAlloc(cimgui_builder.graph.io, cimgui_builder.pathResolve(&.{ "dcimgui", "master", "backends", "imgui_impl_glfw.cpp" }), root.builder.graph.arena, .unlimited);
    imgui_impl_glfw_cpp = std.mem.replaceOwned(
        u8,
        root.builder.graph.arena,
        imgui_impl_glfw_cpp,
        "static ImVector<ImGui_ImplGlfw_WindowToContext> g_ContextMap;",
        \\static ImVector<ImGui_ImplGlfw_WindowToContext>& GetContextMap() {
        \\    static ImVector<ImGui_ImplGlfw_WindowToContext> map;
        \\    return map;
        \\}
        ,
    ) catch @panic("OOM");
    imgui_impl_glfw_cpp = std.mem.replaceOwned(u8, root.builder.graph.arena, imgui_impl_glfw_cpp, "g_ContextMap", "GetContextMap()") catch @panic("OOM");
    const cimgui_dir = cimgui_builder.addWriteFiles();
    _ = cimgui_dir.addCopyFile(cimgui_builder.path(cimgui_builder.pathResolve(&.{ "dcimgui", "master", "backends", "imgui_impl_glfw.h" })), "imgui_impl_glfw.h");
    cimgui_lib.root_module.addCSourceFile(.{ .file = cimgui_dir.add("imgui_impl_glfw.cpp", imgui_impl_glfw_cpp), .flags = &glfw3_flags });

    cimgui_lib.root_module.addCSourceFile(.{ .file = cimgui_builder.path(cimgui_builder.pathResolve(&.{ "dcimgui", "master", "backends", "dcimgui_impl_glfw.cpp" })), .flags = &glfw3_flags });
...
fn importWGSLShaders(root: *Root, module: *std.Build.Module) !void {
    ...
    const spirv2wgsl_mod = root.builder.createModule(.{
        ...
        .optimize = root.native.mode,
        .imports = &.{
            .{ .name = "c", .module = translate_libspirv_and_tint.mod },
        },
    });
    ...
    root.builder.getInstallStep().dependOn(&spirv2wgsl_install.step);
