...
pub const Step = struct {
    ...
    pub fn create(dependency: *std.Build.Dependency, options: Options) !*@This() {
    ...
    fn add(self: @This(), builder: *std.Build, dir: *std.fs.Dir, entry: *const std.fs.Dir.Walker.Entry, dupe: []const u8, ptr: *Node) !void {
        if (!std.mem.eql(u8, entry.basename, dupe)) return;

        const path = builder.dupe(entry.path);
        const source = try dir.readFileAlloc(builder.allocator, path, std.math.maxInt(usize));
    ...
    fn make(step: *std.Build.Step, make_options: std.Build.Step.MakeOptions) !void {
        ...
        try self.shaders_compiler.step.makeFn(&self.shaders_compiler.step, make_options);

        try self.write_index(builder);
    }

    pub fn compileModule(dependency: *std.Build.Dependency, options: Options) !*std.Build.Module {
        const step = try create(dependency, options);
        const builder = dependency.builder;
        return builder.createModule(.{
            .root_source_file = .{
                .generated = .{ .file = &step.generated_file },
            },
