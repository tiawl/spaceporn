...
fn generate_literals(builder: *std.Build, profile: *const Profile) ![]const u8 {
    ...
    var prototypes: struct {
        ...
        source: [:0]const u8,
        path: []const u8,
        structless: std.ArrayList([]const u8),
    ...
    while (try walker.next()) |*entry| {
        ...
        const size: usize = 6;
        var precedent: [size]?std.zig.Token = .{
            null,
        } ** size;
            ...
            if (precedent[precedent.len - 1] != null) {
                if (std.mem.startsWith(u8, binding.buffer.items[token.loc.start..token.loc.end], "vk") and
                    (!profile.options.turbo or (profile.options.turbo and
                        std.mem.indexOf(u8, binding.buffer.items[token.loc.start..token.loc.end], "Debug") == null)) and
...
pub fn import(builder: *std.Build, profile: *const Profile, c: *Package) !*Package {
    ...
    var vk = try Package.init(builder, profile, "vk", try std.fs.path.join(builder.allocator, &.{
        path,
        "vk.zig",
    }));
    try vk.put(c, .{});

    const literals = try Package.init(builder, profile, "literals", try generate_literals(builder, profile));

    var raw = try Package.init(builder, profile, "raw", try std.fs.path.join(builder.allocator, &.{
        path,
        "raw.zig",
    }));
    try raw.put(c, .{});
    ...
    var ext = try Package.init(builder, profile, "ext", try std.fs.path.join(builder.allocator, &.{
        path,
        "ext.zig",
    }));
    try ext.put(c, .{});
    ...
    const ext_path = try std.fs.path.join(builder.allocator, &.{
        path,
        "ext",
    });
    var ext_dir = try builder.build_root.handle.openDir(ext_path, .{
        .iterate = true,
    });
    defer ext_dir.close();
    ...
    while (try walker.next()) |entry| {
        switch (entry.kind) {
            .file => {
                sub = try Package.init(builder, profile, builder.dupe(std.fs.path.stem(entry.basename)), try std.fs.path.join(builder.allocator, &.{
                    ext_path,
                    entry.path,
                }));
                try sub.put(c, .{});
    ...
    var khr = try Package.init(builder, profile, "khr", try std.fs.path.join(builder.allocator, &.{
        path,
        "khr.zig",
    }));
    try khr.put(c, .{});
    ...
    var dir = try builder.build_root.handle.openDir(path, .{
        .iterate = true,
    });
    defer dir.close();
    ...
    while (try it.next()) |entry| {
        switch (entry.kind) {
            .file => {
                ...
                {
                    sub = try Package.init(builder, profile, builder.dupe(std.fs.path.stem(entry.name)), try std.fs.path.join(builder.allocator, &.{
                        path,
                        entry.name,
                    }));
                    try sub.put(c, .{});
