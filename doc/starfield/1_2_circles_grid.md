...
each circle we want to draw. An usual way to achieve this is to draw a grid of
primitives (here our primitive is a circle).

As mentioned in the last section of this tutorial, the `length(v)` builtin
function returns the length of its parameter: the vector `v`. A vector is
expressed by the substraction between its 2 points. Until now, the center of
our circle was the origin. So the other point of each of our vectors was the
...
this unzoom, we have to include our new way to draw circles:
