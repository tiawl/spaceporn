...
float calc_star(vec2 coords, vec2 center, bool anim)
{
  ...
  float power = round(sin((anim ? iTime : 1.) * (3. + 4. * hash(center, SEED + 6u)))) * 0.2 + 1.;
...
vec3 bigstars(vec2 coords, bool anim, bool l)
{
  ...
  return vec3(l ? -d * fv * fv * 0.5 : -d, U * pix);
}
