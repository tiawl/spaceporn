...
float polar(vec2 uv, vec2 center, float size, float brightness, float shape,
  uint sharpness, float diag)
{
  ...
  float m = min(min(smin(s1 - depth * 10., s3, shape, sharpness),
    smin(s2, s3, shape, sharpness)), min(smin(s1 - depth * 10., s4, shape,
      sharpness), smin(s2, s4, shape, sharpness)));

  float color = (sign(m) < .5 ? 1. : 0.);
  vec2 mirror_uv = vec2(abs(uv.x - center.x), 2. * abs(uv.y - center.y));
