...
fn resolve_include(user_data: ?*anyopaque, requested_source: [*c]const u8, @"type": c_int, requesting_source: [*c]const u8, include_depth: usize) callconv(std.builtin.CallingConvention.c) [*c]shaderc.Include.Result {
    ...
    std.log.info("  includer_name: \"{s}\"", .{
        includer_name,
    });
    std.log.info("  includer_depth: {}", .{
        include_depth,
    });
    std.log.info("  header_name: \"{s}\"", .{
        header_name,
    });
    std.log.info("  context.dirname: \"{s}\"", .{
        context.dirname,
    });
...
pub fn main() !void {
    ...
    _ = arg.next().?;
    const optimization = std.meta.stringToEnum(shaderc.OptimizationLevel, arg.next().?).?;
    const env_version = std.meta.stringToEnum(shaderc.Env.VulkanVersion, arg.next().?).?;
    ...
    while (arg.next()) |in| {
        ...
        options.setIncludeCallbacks(resolve_include, release_include, &include_context);

        in_dir = try std.fs.openDirAbsolute(include_context.dirname, .{});
        defer in_dir.close();

        const source = try in_dir.readFileAlloc(allocator, std.fs.path.basename(in), std.math.maxInt(usize));

        const relative = try std.fs.path.relative(allocator, shaders_path.?, in);

        const result = try compiler.compileIntoSpv(allocator, source, kind, relative, options);
        ...
        try out_dir.writeFile(.{
            .sub_path = std.fs.path.basename(out),
            .data = bytes,
        });
    }
}
