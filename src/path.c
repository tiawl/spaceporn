...
bool initPath(char** path, size_t len[2], char* root, char* dir, Log* log)
{
  ...
  do
  {
    writeLog(log, stdout, "", "    Allocating memory for path ...\n");
    ...
    writeLog(log, stdout, "", "    Successful allocated memory for path\n");

    writeLog(log, stdout, "", "    Building path string ... 0/2\n");
    memcpy(*path, root, len[0]);
    writeLog(log, stdout, "", "    Building path string ... 1/2\n");
    memcpy(*path + len[0], dir, len[1] + 1);
    writeLog(log, stdout, "", "    Building path string ... 2/2\n");
    writeLog(log, stdout, "", "    Path string built: %s\n", *path);
  } while (false);
...
bool initPaths(Shaders* shaders, PNG* atlas, Log* log)
{
  ...
  do
  {
    ...
    writeLog(log, stdout, "", "  Fragment shader directory path initialized\n");
    ...
    writeLog(log, stdout, "", "  Vertex shader directory path initialized\n");
