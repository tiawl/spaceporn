...
pub const Instance = struct {
    pub const ProcAddress = struct {
        pub fn get(instance: ?*anyopaque, name: [*:0]const u8) callconv(c.call_conv) ?VKProc {
            if (c.glfwGetInstanceProcAddress(if (instance) |v| @ptrCast(v) else null, name)) |addr| return addr;
            return null;
        }
    };
