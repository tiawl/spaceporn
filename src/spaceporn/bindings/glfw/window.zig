...
pub const Window = struct {
    ...
    pub const Size = struct {
        ...
        pub const Limits = struct {
            pub fn set(min: glfw.Window.Size.Optional, max: glfw.Window.Size.Optional) !void {
                ...
                c.glfwSetWindowSizeLimits(window, if (min.width) |min_width| @as(c_int, @intCast(min_width)) else c.GLFW_DONT_CARE, if (min.height) |min_height| @as(c_int, @intCast(min_height)) else c.GLFW_DONT_CARE, if (max.width) |max_width| @as(c_int, @intCast(max_width)) else c.GLFW_DONT_CARE, if (max.height) |max_height| @as(c_int, @intCast(max_height)) else c.GLFW_DONT_CARE);
            }
        };
    };

    pub const Surface = struct {
        pub fn create(vk_instance: vk.Instance, window: Window, vk_surface_khr: *vk.KHR.Surface) !void {
            const result = c.glfwCreateWindowSurface(@ptrFromInt(@intFromEnum(vk_instance)), window.handle, null, @ptrCast(@alignCast(vk_surface_khr)));
    ...
    pub fn create(width: u32, height: u32, title: [*:0]const u8, monitor: ?glfw.Monitor, share: ?@This(), hints: []const glfw.Window.Hint) !@This() {
        for (hints) |hint| c.glfwWindowHint(hint.tag(), hint.get());
        if (c.glfwCreateWindow(@as(c_int, @intCast(width)), @as(c_int, @intCast(height)), &title[0], if (monitor) |m| m.handle else null, if (share) |w| w.handle else null)) |handle| return from(handle);
