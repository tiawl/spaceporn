...
pub const Device = enum(usize) {
    ...
    pub const Memory = enum(u64) {
        ...
        pub fn allocate(device: vk.Device, p_allocate_info: *const vk.Memory.Allocate.Info) !@This() {
    ...
    pub fn load(self: @This()) !void {
        inline for (std.meta.fields(@TypeOf(raw.prototypes.device))) |field| {
            ...
            const pointer = raw.prototypes.instance.vkGetDeviceProcAddr(self, name) orelse return error.CommandLoadFailure;
            @field(raw.prototypes.device, field.name) = @ptrCast(pointer);
        }
    }

    pub fn create(physical_device: vk.PhysicalDevice, p_create_info: *const vk.Device.Create.Info) !@This() {
    ...
    pub const ExtensionProperties = extern struct {
        pub fn enumerate(physical_device: vk.PhysicalDevice, p_layer_name: ?[*:0]const u8, p_property_count: *u32, p_properties: ?[*]vk.ExtensionProperties) !void {
            const result =
                raw.prototypes.structless.vkEnumerateDeviceExtensionProperties(physical_device, p_layer_name, p_property_count, p_properties);
            if (result > 0) {
                std.debug.print("{s} failed with {} status code\n", .{
                    @typeName(@This()) ++ "." ++ @src().fn_name,
                    result,
