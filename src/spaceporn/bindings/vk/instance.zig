...
pub const Instance = enum(usize) {
    ...
    pub fn load(self: @This()) !void {
        inline for (std.meta.fields(@TypeOf(raw.prototypes.instance))) |field| {
            ...
            const pointer = raw.prototypes.structless.vkGetInstanceProcAddr(self, name) orelse return error.CommandLoadFailure;
    ...
    pub const ExtensionProperties = extern struct {
        pub fn enumerate(p_layer_name: ?[*:0]const u8, p_property_count: *u32, p_properties: ?[*]vk.ExtensionProperties) !void {
            const result =
                raw.prototypes.structless.vkEnumerateInstanceExtensionProperties(p_layer_name, p_property_count, p_properties);
            if (result > 0) {
                std.debug.print("{s} failed with {} status code\n", .{
                    @typeName(@This()) ++ "." ++ @src().fn_name,
                    result,
    ...
    pub const LayerProperties = extern struct {
        pub fn enumerate(p_property_count: *u32, p_properties: ?[*]vk.LayerProperties) !void {
            const result =
                raw.prototypes.structless.vkEnumerateInstanceLayerProperties(p_property_count, p_properties);
            if (result > 0) {
                std.debug.print("{s} failed with {} status code\n", .{
                    @typeName(@This()) ++ "." ++ @src().fn_name,
                    result,
