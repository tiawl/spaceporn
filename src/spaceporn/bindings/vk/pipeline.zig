...
pub const Pipeline = enum(u64) {
    ...
    pub const Layout = enum(u64) {
        ...
        pub fn create(device: vk.Device, p_create_info: *const vk.Pipeline.Layout.Create.Info) !@This() {
...
pub const Graphics = extern struct {
    ...
    pub const Pipelines = extern struct {
        pub fn create(device: vk.Device, pipeline_cache: vk.Pipeline.Cache, create_info_count: u32, p_create_infos: [*]const vk.Graphics.Pipeline.Create.Info, p_pipelines: [*]vk.Pipeline) !void {
            const p_allocator: ?*const vk.AllocationCallbacks = null;
