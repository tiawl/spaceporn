...
const Prototypes = struct {
    fn ziggify(comptime T: type) type {
        ...
        while (start < end) {
            ...
            } else end = std.mem.lastIndexOfAny(u8, name[0 .. end - 1], "ABCDEFGHIJKLMNOPQRSTUVWXYZ") orelse
    ...
    fn cast_rec(comptime T: type, is_opaque: *bool) type {
        ...
        return switch (info) {
            .@"opaque" => blk: {
                if (T == anyopaque) break :blk T else {
                    is_opaque.* = true;
                    break :blk ziggify(T);
                }
            },
            .optional => blk: {
                ...
                if (is_opaque.*) {
                    ...
                    info.optional.child = child;
                    break :blk @Type(info);
                }
            },
            .pointer => blk: {
                const child = cast_rec(info.pointer.child, is_opaque);
                if (is_opaque.*) break :blk child else {
                    info.pointer.child = child;
                    break :blk @Type(info);
                }
            },
