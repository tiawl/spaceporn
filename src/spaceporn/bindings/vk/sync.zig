...
pub const Fence = enum(u64) {
    ...
    pub fn create(device: vk.Device, p_create_info: *const vk.Fence.Create.Info) !@This() {
...
pub const Fences = extern struct {
    pub fn reset(device: vk.Device, fence_count: u32, p_fences: [*]const vk.Fence) !void {
        const result = raw.prototypes.device.vkResetFences(device, fence_count, p_fences);
        if (result > 0) {
            std.debug.print("{s} failed with {} status code\n", .{
                @typeName(@This()) ++ "." ++ @src().fn_name,
                result,
    ...
    pub fn waitFor(device: vk.Device, fence_count: u32, p_fences: [*]const vk.Fence, wait_all: vk.Bool32, timeout: u64) !void {
        const result = raw.prototypes.device.vkWaitForFences(device, fence_count, p_fences, wait_all, timeout);
        if (result > 0) {
            std.debug.print("{s} failed with {} status code\n", .{
                @typeName(@This()) ++ "." ++ @src().fn_name,
                result,
...
pub const Semaphore = enum(u64) {
    ...
    pub fn create(device: vk.Device, p_create_info: *const vk.Semaphore.Create.Info) !@This() {
