...
pub const Context = struct {
    ...
    fn find_queue_families(self: *@This(), device: vk.PhysicalDevice) !?struct {
        graphics_family: u32,
        present_family: u32,
    } {
        ...
        for (queue_families, 0..) |properties, index| {
            ...
            if (vk.Queue.Bit.GRAPHICS.contains(properties.queue_flags) and
                try vk.KHR.PhysicalDevice.Surface.Support.get(device, family, self.surface) == vk.TRUE)
            {
            ...
            if (present_family == null and
                try vk.KHR.PhysicalDevice.Surface.Support.get(device, family, self.surface) == vk.TRUE) present_family = family;
        }
    ...
    fn check_device_extension_support(self: *@This(), device: vk.PhysicalDevice, name: [vk.MAX_PHYSICAL_DEVICE_NAME_SIZE]u8) !bool {
        ...
        try vk.Device.ExtensionProperties.enumerate(device, null, &supported_device_extensions_count, null);
        ...
        try vk.Device.ExtensionProperties.enumerate(device, null, &supported_device_extensions_count, supported_device_extensions.ptr);
        ...
        for (required_device_extensions) |required_ext| {
            for (supported_device_extensions) |supported_ext| {
                if (std.mem.eql(u8, std.mem.span(required_ext), supported_ext.extension_name[0..std.mem.indexOfScalar(u8, &(supported_ext.extension_name), 0).?])) {
                    try self.logger.app(.DEBUG, "Vulkan device {s} supports the {s} required device extension", .{
                        name,
                        required_ext,
                    });
                    break;
                ...
                try self.logger.app(.DEBUG, "Vulkan device {s} does not support the {s} required device extension", .{
                    name,
                    required_ext,
                });
                return false;
        ...
        self.candidate.extensions =
            try std.ArrayList([*:0]const u8).initCapacity(self.logger.allocator.*, required_device_extensions.len);

        try self.candidate.extensions.appendSlice(required_device_extensions[0..]);

        try self.logger.app(.DEBUG, "Vulkan device {s} supports all required device extension", .{
            name,
        });
        return true;
    }
    ...
    fn query_swapchain_support(self: *@This(), device: vk.PhysicalDevice) !void {
        ...
        try vk.KHR.PhysicalDevice.Surface.Formats.get(device, self.surface, &format_count, null);

        if (format_count > 0) {
            self.formats = try self.logger.allocator.alloc(vk.KHR.Surface.Format, format_count);

            try vk.KHR.PhysicalDevice.Surface.Formats.get(device, self.surface, &format_count, self.formats.ptr);
        }

        var present_mode_count: u32 = undefined;

        try vk.KHR.PhysicalDevice.Surface.PresentModes.get(device, self.surface, &present_mode_count, null);

        if (present_mode_count > 0) {
            self.present_modes = try self.logger.allocator.alloc(vk.KHR.Present.Mode, present_mode_count);

            try vk.KHR.PhysicalDevice.Surface.PresentModes.get(device, self.surface, &present_mode_count, self.present_modes.ptr);
        }
    ...
    fn check_device_features_properties(self: @This(), features: vk.PhysicalDevice.Features, properties: vk.PhysicalDevice.Properties) !bool {
        if (features.sampler_anisotropy != vk.TRUE) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support sampler anisotropy feature", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports sampler anisotropy feature", .{
            properties.device_name,
        });

        if (properties.limits.max_sampler_anisotropy < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support a sampler anisotropy value greater than 1", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports a sampler anisotropy value greater than 1", .{
            properties.device_name,
        });

        if (properties.limits.max_image_dimension_2d < self.offscreen_width) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support a maximum 2D image size ({d}) greater than offscreen framebuffer width ({d})", .{
                properties.device_name, properties.limits.max_image_dimension_2d,
                self.offscreen_width,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports a maximum 2D image size ({d}) greater than offscreen framebuffer width ({d})", .{
            properties.device_name, properties.limits.max_image_dimension_2d,
            self.offscreen_width,
        });

        if (properties.limits.max_image_dimension_2d < self.offscreen_height) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support a maximum 2D image size ({d}) greater than offscreen framebuffer height ({d})", .{
                properties.device_name, properties.limits.max_image_dimension_2d,
                self.offscreen_height,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports a maximum 2D image size ({d}) greater than offscreen framebuffer height ({d})", .{
            properties.device_name, properties.limits.max_image_dimension_2d,
            self.offscreen_height,
        });
        ...
        {
            try self.logger.app(.INFO, "Vulkan device {s} does not support a maximum uniform buffer range ({d}) greater than uniform_buffer_object_vk struct size ({d})", .{
                properties.device_name,
        ...
        try self.logger.app(.DEBUG, "Vulkan device {s} supports a maximum uniform buffer range ({d}) greater than uniform_buffer_object_vk struct size ({d})", .{
        ...
        {
            try self.logger.app(.INFO, "Vulkan device {s} does not support a maximum uniform buffer range ({d}) greater than offscreen_uniform_buffer_object_vk struct size ({d})", .{
                properties.device_name,
                properties.limits.max_uniform_buffer_range,
                @sizeOf(offscreen_uniform_buffer_object_vk),
            });
        ...
        try self.logger.app(.DEBUG, "Vulkan device {s} supports a maximum uniform buffer range ({d}) greater than offscreen_uniform_buffer_object_vk struct size ({d})", .{
            properties.device_name,                      properties.limits.max_uniform_buffer_range,
            @sizeOf(offscreen_uniform_buffer_object_vk),
        });

        if (properties.limits.max_memory_allocation_count < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support a vkAllocateMemory() call", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} vkAllocateMemory() calls simultaneously", .{
            properties.device_name,
            properties.limits.max_memory_allocation_count,
        });

        if (properties.limits.max_sampler_allocation_count < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support creation of sampler objects", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} sampler objects simultaneously", .{
            properties.device_name,
            properties.limits.max_sampler_allocation_count,
        });

        if (properties.limits.max_bound_descriptor_sets < 2) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support that 2 or more descriptor sets can be simultaneously used", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} descriptor sets used simultaneously", .{
            properties.device_name,
            properties.limits.max_bound_descriptor_sets,
        });

        if (properties.limits.max_per_stage_descriptor_samplers < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support access to a sample object by a single shader stage", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} accessible sample objects by a single shader stage", .{
            properties.device_name,
            properties.limits.max_per_stage_descriptor_samplers,
        });

        if (properties.limits.max_per_stage_descriptor_uniform_buffers < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support access to a uniform buffer by a single shader stage", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} accessible uniform buffers by a single shader stage", .{
            properties.device_name,
            properties.limits.max_per_stage_descriptor_samplers,
        });

        if (properties.limits.max_per_stage_descriptor_sampled_images < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support access to a sampled image by a single shader stage", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} accessible sampled image by a single shader stage", .{
            properties.device_name,
            properties.limits.max_per_stage_descriptor_sampled_images,
        });

        if (properties.limits.max_per_stage_resources < 2) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support access to 2 or more resources by a single shader stage", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} resources by a single shader stage", .{
            properties.device_name,
            properties.limits.max_per_stage_resources,
        });

        if (properties.limits.max_descriptor_set_uniform_buffers < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support inclusion of a uniform buffer in a pipeline layout", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} included uniform buffers in a pipeline layout", .{
            properties.device_name,
            properties.limits.max_descriptor_set_uniform_buffers,
        });

        if (properties.limits.max_descriptor_set_samplers < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support inclusion of a sampler in a pipeline layout", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} included samplers in a pipeline layout", .{
            properties.device_name,
            properties.limits.max_descriptor_set_samplers,
        });

        if (properties.limits.max_descriptor_set_sampled_images < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support inclusion of a sampled image in a pipeline layout", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} included sampled image in a pipeline layout", .{
            properties.device_name,
            properties.limits.max_descriptor_set_sampled_images,
        });

        if (properties.limits.max_fragment_input_components < 2) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support 2 or more components of input variables provided as inputs to the fragment shader stage", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} components of input variables provided as inputs to the fragment shader stage", .{
            properties.device_name,
            properties.limits.max_fragment_input_components,
        });

        if (properties.limits.max_viewports < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support active viewport", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} active viewports", .{
            properties.device_name,
            properties.limits.max_viewports,
        });
        ...
        if (properties.limits.max_color_attachments < 1) {
            try self.logger.app(.INFO, "Vulkan device {s} does not support color attachment used by a subpass in a render pass", .{
                properties.device_name,
            });
            return false;
        }
        try self.logger.app(.DEBUG, "Vulkan device {s} supports until {d} color attachment used by a subpass in a render pass", .{
            properties.device_name,
    ...
    fn is_suitable(self: *@This(), device: vk.PhysicalDevice) !?struct {
        graphics_family: u32,
        present_family: u32,
        score: u32,
        blitting_supported: bool,
        ...
        const blitting_supported = vk.Format.Feature.Bit.BLIT_SRC.contains(vk.PhysicalDevice.Format.Properties.get(device, selected_format).optimal_tiling_features) and
            vk.Format.Feature.Bit.BLIT_DST.contains(vk.PhysicalDevice.Format.Properties.get(device, .R8G8B8A8_UNORM).linear_tiling_features);
    ...
    fn pick_physical_device(self: *@This()) !void {
        ...
        try vk.PhysicalDevices.enumerate(self.instance.instance, &device_count, null);
        ...
        try vk.PhysicalDevices.enumerate(self.instance.instance, &device_count, devices.ptr);
    ...
    fn init_logical_device(self: *@This()) !void {
        ...
        self.logical_device = try vk.Device.create(self.physical_device.?, &device_create_info);
    ...
    fn choose_swap_extent(self: *@This(), framebuffer: struct {
        width: u32,
        height: u32,
    }) void {
    ...
    fn init_swapchain_images(self: *@This()) !void {
        ...
        try vk.KHR.Swapchain.Images.get(self.logical_device, self.swapchain, &image_count, null);
        ...
        try vk.KHR.Swapchain.Images.get(self.logical_device, self.swapchain, &image_count, self.images.ptr);

        try self.logger.app(.DEBUG, "init Vulkan swapchain images OK", .{});
    }

    fn init_swapchain(self: *@This(), framebuffer: struct {
        width: u32,
        height: u32,
    }) !void {
        self.choose_swap_support_format();
        const present_mode = self.choose_swap_present_mode();
        self.choose_swap_extent(.{
        ...
        self.swapchain = try vk.KHR.Swapchain.create(self.logical_device, &create_info);
    ...
    fn init_image_views(self: *@This()) !void {
        ...
        for (self.images, 0..) |image, index| {
            ...
            self.views[index] = try vk.Image.View.create(self.logical_device, &create_info);
    ...
    fn init_render_pass(self: *@This()) !void {
        ...
        self.render_pass = try vk.RenderPass.create(self.logical_device, &create_info);
        errdefer self.render_pass.destroy(self.logical_device);

        try self.logger.app(.DEBUG, "init Vulkan render pass OK", .{});
    }

    fn find_memory_type(self: @This(), type_filter: u32, properties: vk.Memory.Property.Flags) !u32 {
        const memory_properties = vk.PhysicalDevice.Memory.Properties.get(self.physical_device.?);
    ...
    fn init_offscreen(self: *@This()) !void {
        ...
        self.offscreen_image = try vk.Image.create(self.logical_device, &image_create_info);
        ...
        self.offscreen_image_memory = try vk.Device.Memory.allocate(self.logical_device, &alloc_info);
        ...
        self.offscreen_views[0] = try vk.Image.View.create(self.logical_device, &view_create_info);
        ...
        self.offscreen_sampler = try vk.Sampler.create(self.logical_device, &sampler_create_info);
        ...
        self.offscreen_render_pass = try vk.RenderPass.create(self.logical_device, &create_info);
        ...
        self.offscreen_framebuffer = try vk.Framebuffer.create(self.logical_device, &framebuffer_create_info);
    ...
    fn init_descriptor_set_layout(self: *@This()) !void {
        ...
        self.descriptor_set_layout[0] = try vk.Descriptor.Set.Layout.create(self.logical_device, &create_info);
        ...
        errdefer self.offscreen_descriptor_set_layout[0].destroy(self.logical_device);
    ...
    fn init_graphics_pipeline(self: *@This()) !void {
        ...
        try vk.Graphics.Pipelines.create(self.logical_device, .NULL_HANDLE, pipeline_create_info.len, &pipeline_create_info, self.pipelines.ptr);
        errdefer {
            var index: u32 = 0;
        ...
        try vk.Graphics.Pipelines.create(self.logical_device, .NULL_HANDLE, pipeline_create_info.len, &pipeline_create_info, self.offscreen_pipelines.ptr);
        errdefer {
            var index: u32 = 0;
    ...
    fn init_framebuffers(self: *@This()) !void {
        ...
        for (self.framebuffers) |*framebuffer| {
            ...
            framebuffer.* = try vk.Framebuffer.create(self.logical_device, &create_info);
    ...
    fn init_command_pools(self: *@This()) !void {
        ...
        self.command_pool = try vk.Command.Pool.create(self.logical_device, &create_info);
        ...
        self.buffers_command_pool = try vk.Command.Pool.create(self.logical_device, &buffers_create_info);
    ...
    fn init_buffer(self: @This(), size: vk.Device.Size, usage: vk.Buffer.Usage.Flags, properties: vk.Memory.Property.Flags, buffer: *vk.Buffer, buffer_memory: *vk.Device.Memory) !void {
        ...
        // TODO: issue #68
        buffer_memory.* = try vk.Device.Memory.allocate(self.logical_device, &alloc_info);
    ...
    fn copy_buffer(self: @This(), src_buffer: vk.Buffer, dst_buffer: vk.Buffer, size: vk.Device.Size) !void {
        var command_buffers = [_]vk.Command.Buffer{
            undefined,
        };
    ...
    fn init_vertex_buffer(self: *@This()) !void {
        ...
        try self.init_buffer(size, @intFromEnum(vk.Buffer.Usage.Bit.TRANSFER_SRC), @intFromEnum(vk.Memory.Property.Bit.HOST_VISIBLE) |
            @intFromEnum(vk.Memory.Property.Bit.HOST_COHERENT), &staging_buffer, &staging_buffer_memory);
        ...
        try self.init_buffer(size, @intFromEnum(vk.Buffer.Usage.Bit.TRANSFER_DST) |
            @intFromEnum(vk.Buffer.Usage.Bit.VERTEX_BUFFER), @intFromEnum(vk.Memory.Property.Bit.DEVICE_LOCAL), &(self.vertex_buffer), &(self.vertex_buffer_memory));
    ...
    fn init_index_buffer(self: *@This()) !void {
        ...
        try self.init_buffer(size, @intFromEnum(vk.Buffer.Usage.Bit.TRANSFER_DST) |
            @intFromEnum(vk.Buffer.Usage.Bit.INDEX_BUFFER), @intFromEnum(vk.Memory.Property.Bit.DEVICE_LOCAL), &(self.index_buffer), &(self.index_buffer_memory));
    ...
    fn init_uniform_buffers(self: *@This()) !void {
        ...
        while (index < MAX_FRAMES_IN_FLIGHT) {
            try self.init_buffer(@sizeOf(uniform_buffer_object_vk), @intFromEnum(vk.Buffer.Usage.Bit.UNIFORM_BUFFER), @intFromEnum(vk.Memory.Property.Bit.HOST_VISIBLE) |
                @intFromEnum(vk.Memory.Property.Bit.HOST_COHERENT), &(self.uniform_buffers[index]), &(self.uniform_buffers_memory[index]));
            index += 1;
        }
        ...
        try self.init_buffer(@sizeOf(offscreen_uniform_buffer_object_vk), @intFromEnum(vk.Buffer.Usage.Bit.UNIFORM_BUFFER), @intFromEnum(vk.Memory.Property.Bit.HOST_VISIBLE) |
            @intFromEnum(vk.Memory.Property.Bit.HOST_COHERENT), &(self.offscreen_uniform_buffers), &(self.offscreen_uniform_buffers_memory));
    ...
    fn init_descriptor_pool(self: *@This()) !void {
        ...
        self.descriptor_pool = try vk.Descriptor.Pool.create(self.logical_device, &create_info);
    ...
    fn init_sync_objects(self: *@This()) !void {
        ...
        while (index < MAX_FRAMES_IN_FLIGHT) {
            self.image_available_semaphores[index] = try vk.Semaphore.create(self.logical_device, &vk.Semaphore.Create.Info{});
            errdefer self.image_available_semaphores[index].destroy(self.logical_device);
            self.render_finished_semaphores[index] = try vk.Semaphore.create(self.logical_device, &vk.Semaphore.Create.Info{});
            errdefer self.render_finished_semaphores[index].destroy(self.logical_device);
            self.in_flight_fences[index] = try vk.Fence.create(self.logical_device, &vk.Fence.Create.Info{
    ...
    pub fn get_surface(self: @This()) struct {
    ...
    pub fn init(self: *@This(), imgui: ImguiContext, framebuffer: struct {
        width: u32,
        height: u32,
    }) !void {
    ...
    fn record_command_buffer(self: *@This(), imgui: *ImguiContext, command_buffer: *vk.Command.Buffer, image_index: u32) !void {
        ...
        const offset = [_]vk.Device.Size{
            0,
        };
        command_buffer.bind_vertex_buffers(0, 1, &[_]vk.Buffer{
            self.vertex_buffer,
        }, &offset);
        ...
        if (self.render_offscreen) {
            command_buffer.bind_descriptor_sets(.GRAPHICS, self.offscreen_pipeline_layout, 0, 1, self.offscreen_descriptor_sets.ptr, 0, undefined);
        ...
        command_buffer.bind_descriptor_sets(.GRAPHICS, self.pipeline_layout, 0, 1, &[_]vk.Descriptor.Set{
    ...
    fn rebuild_swapchain(self: *@This(), framebuffer: struct {
        width: u32,
        height: u32,
    ...
    fn update_uniform_buffer(self: *@This(), options: *Options) !void {
        ...
        var data = try self.uniform_buffers_memory[self.current_frame].map(self.logical_device, 0, ubo_size, 0);
        defer self.uniform_buffers_memory[self.current_frame].unmap(self.logical_device);
        ...
        if (self.render_offscreen) {
            ...
            data = try self.offscreen_uniform_buffers_memory.map(self.logical_device, 0, oubo_size, 0);
    ...
    fn find_available_file(self: @This(), allocator: std.mem.Allocator, dir: std.fs.Dir) ![]const u8 {
        ...
        while (iterator.next()) |token| {
            date = try std.fmt.allocPrint(allocator, "{s}{s}-", .{
                date,
                token,
            });
        }
        ...
        while (id < std.math.maxInt(u32)) {
            filename = try std.fmt.allocPrint(allocator, "{s}{d}", .{
                date,
                id,
            });
        ...
        filename = try std.fmt.allocPrint(allocator, "{s}.ppm", .{
            filename,
        });
        return filename;
    }

    fn save_screenshot_to_disk(self: @This(), allocator: std.mem.Allocator, framebuffer: struct {
        width: u32,
        height: u32,
    }) !void {
        ...
        const dst_image = try vk.Image.create(self.logical_device, &image_create_info);
        defer dst_image.destroy(self.logical_device);

        const memory_requirements = vk.Image.Memory.Requirements.get(self.logical_device, dst_image);

        const alloc_info = vk.Memory.Allocate.Info{
            .allocation_size = memory_requirements.size,
            .memory_type_index = try self.find_memory_type(memory_requirements.memory_type_bits, @intFromEnum(vk.Memory.Property.Bit.HOST_VISIBLE) |
                @intFromEnum(vk.Memory.Property.Bit.HOST_COHERENT)),
        };

        const dst_image_memory = try vk.Device.Memory.allocate(self.logical_device, &alloc_info);
        errdefer dst_image_memory.free(self.logical_device);

        try vk.Image.Memory.bind(self.logical_device, dst_image, dst_image_memory, 0);
        ...
        try vk.Command.Buffers.allocate(self.logical_device, &buffers_alloc_info, &command_buffers);
        ...
        command_buffers[0].pipeline_barrier(@intFromEnum(vk.Pipeline.Stage.Bit.TRANSFER), @intFromEnum(vk.Pipeline.Stage.Bit.TRANSFER), 0, 0, null, 0, null, 1, &dst_image_to_transfer_dst_layout);
        ...
        command_buffers[0].pipeline_barrier(@intFromEnum(vk.Pipeline.Stage.Bit.TRANSFER), @intFromEnum(vk.Pipeline.Stage.Bit.TRANSFER), 0, 0, null, 0, null, 1, &swapchain_image_from_present_to_transfer_src_layout);
        ...
        if (self.candidate.blitting_supported) {
            ...
            command_buffers[0].blit_image(self.images[self.screenshot_image_index], .TRANSFER_SRC_OPTIMAL, dst_image, .TRANSFER_DST_OPTIMAL, 1, &image_blit_region, .NEAREST);
        } else {
            ...
            command_buffers[0].copy_image(self.images[self.screenshot_image_index], .TRANSFER_SRC_OPTIMAL, dst_image, .TRANSFER_DST_OPTIMAL, 1, &image_copy_region);
        }
        ...
        command_buffers[0].pipeline_barrier(@intFromEnum(vk.Pipeline.Stage.Bit.TRANSFER), @intFromEnum(vk.Pipeline.Stage.Bit.TRANSFER), 0, 0, null, 0, null, 1, &dst_image_to_general_layout);
        ...
        command_buffers[0].pipeline_barrier(@intFromEnum(vk.Pipeline.Stage.Bit.TRANSFER), @intFromEnum(vk.Pipeline.Stage.Bit.TRANSFER), 0, 0, null, 0, null, 1, &swapchain_image_after_blit);
        ...
        const fence = try vk.Fence.create(self.logical_device, &vk.Fence.Create.Info{});
        defer fence.destroy(self.logical_device);

        try self.graphics_queue.submit(1, &submit_info, fence);

        try vk.Fences.waitFor(self.logical_device, 1, &[_]vk.Fence{
            fence,
        ...
        const subresource_layout = vk.Image.Subresource.Layout.get(self.logical_device, dst_image, &subresource);

        // TODO: change as ([*] u8, ...) depending of blit support + surface format
        var data = @as([*]u8, @ptrCast((try dst_image_memory.map(self.logical_device, 0, vk.WHOLE_SIZE, 0)).?));
    ...
    fn draw_frame(self: *@This(), imgui: *ImguiContext, framebuffer: struct {
        resized: bool,
        width: u32,
        height: u32,
    }, arena: *std.heap.ArenaAllocator, allocator: *std.mem.Allocator, options: *Options) !void {
        try vk.Fences.waitFor(self.logical_device, 1, &[_]vk.Fence{
        ...
        if (self.screenshot_frame == std.math.maxInt(u32)) {
            var tweak_me: Tweaker = .{
                .seed = &(options.seed),
            };
            const seed_before = options.seed;
            prepare = try imgui.prepare(allocator, &(self.last_displayed_fps), &(self.fps), .{
                .width = framebuffer.width,
                .height = framebuffer.height,
            }, &tweak_me);
        ...
        const image_index = vk.KHR.NextImage.acquire(self.logical_device, self.swapchain, std.math.maxInt(u64), self.image_available_semaphores[self.current_frame], .NULL_HANDLE) catch |err| switch (err) {
            error.OutOfDateKHR => {
        ...
        try vk.Fences.reset(self.logical_device, 1, &[_]vk.Fence{
            self.in_flight_fences[self.current_frame],
        });

        if (prepare == .Screenshot) self.screenshot_image_index = image_index;
        try self.record_command_buffer(imgui, &(self.command_buffers[self.current_frame]), image_index);
        ...
        try self.graphics_queue.submit(1, &submit_info, self.in_flight_fences[self.current_frame]);
        ...
        const present_info = vk.KHR.Present.Info{
            ...
            .swapchain_count = 1,
            .p_swapchains = &[_]vk.KHR.Swapchain{
                self.swapchain,
            },
            .p_image_indices = &[_]u32{
                image_index,
            },
            .p_results = null,
        };
    ...
    pub fn loop(self: *@This(), imgui: *ImguiContext, framebuffer: struct {
        resized: bool,
        width: u32,
        height: u32,
    }, arena: *std.heap.ArenaAllocator, allocator: *std.mem.Allocator, options: *Options) !void {
        try self.draw_frame(imgui, .{
