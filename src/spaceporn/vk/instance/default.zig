...
pub const instance_vk = struct {
    ...
    fn debug_callback(message_severity: vk.EXT.DebugUtils.Message.Severity.Flags, message_type: vk.EXT.DebugUtils.Message.Type.Flags, p_callback_data: ?*const vk.EXT.DebugUtils.Messenger.Callback.Data, p_user_data: ?*anyopaque) bool {
        const logger: *Logger = @ptrCast(@alignCast(p_user_data));

        logger.vk(if (vk.EXT.DebugUtils.Message.Severity.Bit.VERBOSE.contains(message_severity)) .DEBUG else if (vk.EXT.DebugUtils.Message.Severity.Bit.INFO.contains(message_severity)) .INFO else if (vk.EXT.DebugUtils.Message.Severity.Bit.WARNING.contains(message_severity)) .WARNING else .ERROR, if (vk.EXT.DebugUtils.Message.Type.Bit.GENERAL.contains(message_type)) .GENERAL else if (vk.EXT.DebugUtils.Message.Type.Bit.VALIDATION.contains(message_type)) .VALIDATION else if (vk.EXT.DebugUtils.Message.Type.Bit.PERFORMANCE.contains(message_type)) .PERFORMANCE else .@"DEVICE ADDR BINDING", "{s}", .{p_callback_data.?.p_message}) catch return false;
    ...
    fn init_debug_info(self: @This(), debug_info: *vk.EXT.DebugUtils.Messenger.Create.Info) void {
        ...
        debug_info.* = .{
            .message_severity = (@intFromEnum(vk.EXT.DebugUtils.Message.Severity.Bit.VERBOSE) &
            ...
            .message_type = @intFromEnum(vk.EXT.DebugUtils.Message.Type.Bit.GENERAL) |
                ...
                (@intFromEnum(vk.EXT.DebugUtils.Message.Type.Bit.PERFORMANCE) &
                    @intFromBool(Logger.build.profile.eql(.DEV))) |
                if (@hasField(OptionalExtensions(), vk.EXT.DEVICE_ADDRESS_BINDING_REPORT) and @field(optional_extensions, vk.EXT.DEVICE_ADDRESS_BINDING_REPORT))
    ...
    fn check_extension_properties(self: *@This(), init_extensions: *[][*:0]const u8) ![][*:0]const u8 {
        var extensions =
        ...
        inline for (std.meta.fields(OptionalExtensions())) |*optional_ext| {
            ...
            for (supported_extensions) |supported_ext| {
                if (std.mem.eql(u8, supported_ext.extension_name[0..std.mem.indexOfScalar(u8, &(supported_ext.extension_name), 0).?], optional_ext.name)) {
                ...
                try self.logger.app(.WARNING, "{s} optional extension is not supported", .{
                    optional_ext.name,
                });
            }
        }
