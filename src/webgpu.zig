...
pub fn deviceCreateBindGroup(c_device: c.WGPUDevice, c_descriptor_opt: ?*const c.WGPUBindGroupDescriptor) callconv(.c) c.WGPUBindGroup {
    ...
    if (c_descriptor_opt) |c_descriptor| {
        var bind_group_entries = gpa.alloc(js.gpu.BindGroupEntry, c_descriptor.entryCount) catch std.debug.panic("{s}: out of memory", .{@src().fn_name});
...
pub fn deviceCreateBindGroupLayout(c_device: c.WGPUDevice, c_descriptor_opt: ?*const c.WGPUBindGroupLayoutDescriptor) callconv(.c) c.WGPUBindGroupLayout {
    _ = c_device;
    if (c_descriptor_opt) |c_descriptor| {
        var bind_group_layout_entries = gpa.alloc(js.gpu.BindGroupLayoutEntry, c_descriptor.entryCount) catch std.debug.panic("{s}: out of memory", .{@src().fn_name});
...
pub fn deviceCreatePipelineLayout(c_device: c.WGPUDevice, c_descriptor_opt: ?*const c.WGPUPipelineLayoutDescriptor) callconv(.c) c.WGPUPipelineLayout {
    _ = c_device;
    if (c_descriptor_opt) |c_descriptor| {
        const pipeline_layout = gpa.create(js.gpu.PipelineLayout) catch std.debug.panic("{s}: out of memory", .{@src().fn_name});
        var bind_group_layouts = gpa.alloc(js.gpu.BindGroupLayout, c_descriptor.bindGroupLayoutCount) catch std.debug.panic("{s}: out of memory", .{@src().fn_name});
        defer gpa.free(bind_group_layouts);
...
pub fn deviceCreateRenderPipeline(c_device: c.WGPUDevice, c_descriptor_opt: ?*const c.WGPURenderPipelineDescriptor) callconv(.c) c.WGPURenderPipeline {
    _ = c_device;
    if (c_descriptor_opt) |c_descriptor| {
        const vertex_state = jsVertexStateFromC(gpa, &c_descriptor.vertex);
        defer {
            ...
            for (0..vertex_state.buffers_len) |i| {
                ...
                gpa.free(vertex_attributes_ptr[0..vertex_buffer_layouts_ptr[i].vertex_attributes_len]);
            }
            gpa.free(vertex_buffer_layouts_ptr[0..vertex_state.buffers_len]);
        }

        const fragment_state = jsFragmentStateFromC(gpa, c_descriptor.fragment);
        defer {
            const color_target_states_ptr: [*]js.gpu.ColorTargetState = @ptrFromInt(fragment_state.targets_ptr);
            gpa.free(color_target_states_ptr[0..fragment_state.targets_len]);
        }
...
pub fn textureCreateView(c_texture: c.WGPUTexture, c_descriptor_opt: ?*const c.WGPUTextureViewDescriptor) callconv(.c) c.WGPUTextureView {
    if (c_descriptor_opt) |c_descriptor| {
        ...
        const texture_view = gpa.create(js.gpu.TextureView) catch std.debug.panic("{s}: out of memory", .{@src().fn_name});
